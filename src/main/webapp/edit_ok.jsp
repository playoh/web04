...
<%
    ...
    if (ServletFileUpload.isMultipartContent(request)) {
        try {
            ...
            for (FileItem item : items) {
                if (item.isFormField()) {
                    ...
                    if ("seq".equals(fieldName)) {
                        vo.setSeq(Integer.parseInt(value));
                        ...
                        vo.setContent(value);
                    } else if ("old_photo".equals(fieldName)) {
                        old_photo = value;
                    }
                } else {
                    String fieldName = item.getFieldName();
                    ...
                    if (fieldName.equals("photo") && item.getSize() > 0) {
                        ...
                        // 새 파일이 업로드되지 않은 경우, 기존 파일명 유지
                        if (vo.getPhoto() == null || vo.getPhoto().isEmpty()) {
                            vo.setPhoto(old_photo);
                        }
                    }
                }
            }
            ...
            out.println("<h2>파일 업로드 또는 수정 중 오류 발생</h2>");
            out.println("<hr>");
